...
extern "C"
{
    unsigned char *readImg(unsigned char *addr, int width, int height)
    {
        ...
        if ((img = (unsigned char *)malloc((width * height * 3) * sizeof(unsigned char))) == NULL)
    ...
    unsigned char *resizeImg(unsigned char *bmpImg, int width_img, int height_img, int pixelsToCrop)
    {
        ...
        if ((bmpImgResized = (unsigned char *)malloc((width_img * height_img * 3 - pixelsToCrop * 3) * sizeof(unsigned char))) == NULL)
            printf("erreur allocation mémoire \n");
        ...
        for (int idx_out_height = 0; idx_out_height < out_heigth; idx_out_height++)
        {
            for (int idx_out_width = 0; idx_out_width < out_width; idx_out_width++)
            {
                for (int color = 0; color < colors; color++)
                {
                    ...
                    bmpImgResized[(idx_out_height * out_width + idx_out_width) * colors + color] = bmpImg[idx_out_height * width_img * colors + idx_in_width * colors + color];
                }
            }
        }
    ...
    {
        float *imgRescaled;
        if ((imgRescaled = (float *)malloc(height * width * sizeof(float))) == NULL)
            printf("erreur allocation mémoire \n");
    ...
    void encodageBMP(unsigned char *addr, int width, int height)
    {
        ...
        if ((fileheader = (BITMAPFILEHEADER *)malloc(sizeof(BITMAPFILEHEADER))) == NULL)
            printf("erreur allocation mémoire \n");

        BITMAPINFOHEADER *infoheader;
        if ((infoheader = (BITMAPINFOHEADER *)malloc(sizeof(BITMAPINFOHEADER))) == NULL)
            printf("erreur allocation mémoire \n");
    ...
    int encodeInCSV(unsigned char *img, int length)
    {
        ...
        char *tab = (char *)calloc(sizeof(char), 4 * length);
        char *ptr = tab;
        char retourLigne = '\n';
        char virgule = ',';
        int size = 0;
...
extern "C++"
{
    void preprocess(string filename)
    {
        ...
        unsigned char *img = LoadBitmapFile(file, bitmapinfoheader);
